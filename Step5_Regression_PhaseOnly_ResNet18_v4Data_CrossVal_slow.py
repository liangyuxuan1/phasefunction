...
if __name__=='__main__':
    ...
    for num_of_Gaussian in range(9, 11):
        ...
        for gV in enumerate(g):
            ...
            checkpoint_path = 'training_results_K_%d' % num_of_Gaussian
            if not os.path.exists(checkpoint_path):
                os.mkdir(checkpoint_path)
            writer = SummaryWriter(os.path.join(checkpoint_path,'events_%d' % gV[0]))
            
            loss  = np.zeros((n_epochs, 3))
            since = time.time()
            for epoch in range(n_epochs):
                print(f"NoG:{num_of_Gaussian}, fold:{gV[0]}, Epoch:{epoch+1}")
        ...
        print(df_loss_best)
        print('\n')
        df_loss_best.to_csv(os.path.join(checkpoint_path, 'best_model_results_NoG_%d.csv' % num_of_Gaussian), index=False)
