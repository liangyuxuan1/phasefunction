...
dstDataPath = 'imageCW_v3_Bottom';      % path to store the images
if ~exist(dstDataPath, 'dir')
    mkdir(dstDataPath);
end

changeDataList(srcDataPath, 'trainDataCW_v3_image_Bottom.csv', dstDataPath);
changeDataList(srcDataPath, 'testDataCW_v3_image_Bottom.csv',  dstDataPath);
...
function changeDataList(srcDataPath, dataListFileName, dstDataPath)
    ...
    for i = 1:height(T)
        ...
        rawData = ReadRawData_CW_Bottom(rawDataFileName);
