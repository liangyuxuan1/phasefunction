...
class NeuralNetwork(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            nn.Linear(128, 3*num_of_Gaussian),
            nn.Sigmoid()
        )
...
if __name__=='__main__':
    ...
    trainDataListFile = "trainDataCW_v3_ExcludeExtremes.csv"
    ...
    model = resnet14(pretrained=False, num_classes=num_of_Gaussian*3).to(device)
    # print(model)
    ...
    for epoch in range(start_epoch, n_epochs+1):
        ...
        val_loss, val_correct = test(val_dataloader, model, loss_fn)

        writer.add_scalar('Train/Loss', train_loss, epoch)
        writer.add_scalar('Validation/Loss', val_loss, epoch)
        #writer.add_scalar('Accuracy: relative error < 10%', 100*val_correct[0], epoch)
        #writer.add_scalar('Accuracy: relative error 10-50%', 100*val_correct[1], epoch)
        #writer.add_scalar('Accuracy: relative error > 50%', 100*val_correct[2], epoch)
    ...
    model, optimizer, start_epoch, val_loss_min = load_ckp(best_model_file, model, optimizer)
